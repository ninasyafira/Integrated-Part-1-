...
import danielmainprocess as mp
import Process
import MainProcess
...
def pctransaction():
    ...
    return render_template('pctransaction.html',child=userslist, count=len(userslist),
                          ...
                          monthPrev =MainProcess.previous_Month(), TEST= MainProcess.TEST(session['userid']))
...
def editamount():
    ...
    if request.method == 'POST' and form.validate():
        ...
        print('Success')
        return redirect(url_for('pctransaction'))
    return render_template('editamount.html', form=form)
...
def delete():
    ...
    if request.method == 'POST' and form.validate():
        ...
        return redirect(url_for('pctransaction'))
    return render_template('deletechild.html', form=form)
...
def selected():
    ...
    if request.method == 'POST' and form.validate():
        ...
        return render_template('selected.html',users=usersList,checkMM=months[checkMonth], count=len(usersList),limits=limit,over=over,form=form)
...
def update_savingHistory(postlist):
    ...
    form = MonthForm(request.form)
    return render_template('nov.html',users=usersList,checkMM=checkMM,limits=limit,over=over,user=name,displayHistory=displayHistory,form=form)
...
def rewards():
    ...
    return render_template('rewards.html', users=userlist, rewards=rewardlist, count=len(userlist),userid=session['userid'],userpoint=userpoint)
...
def update_userReward(postlist):
    ...
    rewardlist = []
    rewardlist = mp.processavail_rewards(userpoint)
    return render_template('rewards.html', users=userlist, rewards=rewardlist, count=len(userlist),userid=userid,userpoint=userpoint)
...
def mk():
    ...
    return render_template('Goals.html', form = form, users = userslist, count=len(userslist), user = userlist,
                           ...
                           usersid = useridlist, months = months, form2 = form2)
...
def delete_goal(id):
    ...
    return render_template('Goals.html', form = form, users = userslist, count=len(userslist), user = userlist,
                           ...
                           usersid = useridlist, months = months, users2 = userslist2, countList = len(countList))
...
def checkspend():
    ...
    todayYear = todayDate.year
    todayLong = todayDate.strftime('%d-%B-%Y %H:%M:%S')
    monthDesc = ('null', 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'Decemeber')
    print(monthDesc[1])
...
def formlimit():
    ...
    checkMonth = int(monthMM)
    checkYear = int(yearYY)
    spendMax = mp.processSpendMax(session['userid'], checkMonth, checkYear)
    print(spendMax)

    spendlist = mp.processSpend(session['userid'], checkMonth, checkYear)
    spendtotal = 0
    for s in spendlist:
        print(s)
        spendtotal += s.get_amount()
